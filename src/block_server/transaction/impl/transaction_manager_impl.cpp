...
void TransactionManagerImpl::run() {
    ...
    while(!finishSignal.load(std::memory_order_relaxed)) {
        auto& currentEpochTxQueue = epochTxBuffer[currentEpoch];
        // get all epoch = i transaction
        // note: min size must set to 0, to recv epoch increase signal
        while(trWrapper = commModule->getTransaction(currentEpoch, TR_MGR_GET_BATCH_MAX_SIZE, 0), trWrapper!= nullptr) {
...
void TransactionManagerImpl::epochFinishedCallback(epoch_size_t finishedEpoch) {
    ...
    categorizeTransaction ? categorizeEpoch(finishedEpoch) : nonCategorizeEpoch(finishedEpoch);
    // generate next block
    ...
    client->sendRequest(std::to_string(finishedEpoch)); // we dont need to wait until block is generated.
