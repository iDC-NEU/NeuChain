...
void AriaWorker::run() {
    ...
    while(true) {
        ...
        // reservation phase
        if(!self->workerWait(AriaGlobalState::Aria_RESERVATION)) {
            DLOG(INFO) << "worker id = " << self->workerID  << " recv exit signal";
            self->setWorkerState(WorkerState::EXITED);
            return;
        }
        ...
        transactionExecutor->reserveList(self->workload->transactionList);  // get rw set for each transaction

        DLOG(INFO) << "worker id = " << self->workerID  << " has finished reserve, epoch: " << self->workload->epoch;
        self->setWorkerState(WorkerState::FINISH_RESERVATION);
