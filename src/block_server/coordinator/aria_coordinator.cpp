...
void AriaCoordinator::run() {
    ...
    while(!finishSignal) {
        ...
        readPhase();
        startReservation.wait([&] {
            return this->currentEpoch <= this->reservationEpoch;
        });
        // reservation rw set
        broadcastToAllWorkers(AriaGlobalState::Aria_RESERVATION);
        syncAllWorker({ WorkerState::FINISH_RESERVATION });
