...
raft::ChainClient::ChainClient()
    :latency_recorder("chain_client") { }

const char* raft::ChainClient::group_name = "chain_group";

bool raft::ChainClient::push_user_request(const braft::PeerId& leader, const PushRequest* request, ChainResponse* response) {
...
bool raft::ChainClient::get_leader_from_route_table(braft::PeerId &leader) {
    // Select leader of the target group from RouteTable
    if (braft::rtb::select_leader(group_name, &leader) != 0) {
        // Leader is unknown in RouteTable. Ask RouteTable to refresh leader
        // by sending RPCs.
        butil::Status st = braft::rtb::refresh_leader(group_name, 1000);
        if (!st.ok()) {
...
bool raft::ChainClient::update_leader_in_route_table(const braft::PeerId &leader) {
    braft::rtb::update_leader(group_name, leader);
    return true;
}
