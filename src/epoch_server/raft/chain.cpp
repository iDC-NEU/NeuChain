...
raft::Chain::Chain() : _leader_term(-1) {
...
void
raft::Chain::push(const raft::PushRequest *request, raft::ChainResponse *response, google::protobuf::Closure *done) {
    ...
    if (request->id() != leader_group_id) {
        LOG(ERROR) << "push message to wrong leader group.";
        response->set_success(false);
        return;
    }

    // Serialize request to IOBuf
//    const int64_t term = _leader_term.load(butil::memory_order_acquire);
//    if (term < 0) {
...
//    }
    ...
    node_list[leader_group_id]->apply(task);
}
...
void raft::Chain::on_apply(braft::Iterator &iter) {
    const auto requestHandler = [this](const PushRequest *request) {
        ...
        DLOG(INFO) << " raft::Chain::on_apply receive a batch from raft, size: " << request->user_request_size();
    };
...
bool raft::Chain::add_chain_instance(const std::string &initial_conf, const std::string &ip, int port, const char *group_name, int group_id) {
    ...
    std::string prefix = "local://./raft_data" + std::string(group_name);    //Path of data stored on
    ...
    auto *node = new braft::Node(group_name, braft::PeerId(point, group_id));
    if (node->init(node_options) != 0) {
        LOG(ERROR) << "Fail to init raft node";
        delete node;
        return false;
    }
    CHECK((int)node_list.size() == group_id);
