...
//// we send request to the last block server and received from the first block server
//// the first block server and the client must in the same az
BlockBench::DBUserBase::DBUserBase() :trCount(0) {
    auto *yamlConfig = YAMLConfig::getInstance();
    const auto receivedIP = yamlConfig->getBlockServerIPs()[0];
    const auto sendIP = yamlConfig->getBlockServerIPs().back();
    queryClient = std::make_unique<ZMQClient>(receivedIP, "7003");
    // WE CAN NOT PULL FROM EPOCH SERVER, because it is faster
    // CHECK(yamlConfig->getBlockServerIPs().back() != yamlConfig->getLocalBlockServerIP());
    /*if (yamlConfig->sendToAllClientProxy()) {
        ...
        invokeClient.emplace_back(yamlConfig->getLocalBlockServerIP(), std::make_unique<ZMQClient>(sendIP, "9002", zmq::socket_type::pub));
        sendInvokeRequest = [=](const Utils::Request &request) {
            std::string &&payloadRaw = Utils::getTransactionPayloadRaw(trCount++, request);
            auto &&invokeRequest = Utils::getUserInvokeRequest(payloadRaw, yamlConfig->getLocalBlockServerIP());
            addPendingTransactionHandle(invokeRequest.digest());
...
std::queue<BlockBench::DBUserBase::pollTxType> BlockBench::DBUserBase::pollTx(epoch_size_t block_number) {
    ...
    queryClient->sendRequest(Utils::getUserQueryRequestRaw("block_query", std::to_string(block_number)));
    std::string replyRaw;
    queryClient->getReply(replyRaw);
    Block::Block block;
...
size_t BlockBench::DBUserBase::getTipBlockNumber() {
    queryClient->sendRequest(Utils::getUserQueryRequestRaw("tip_query", ""));
    std::string replyRaw;
    queryClient->getReply(replyRaw);
