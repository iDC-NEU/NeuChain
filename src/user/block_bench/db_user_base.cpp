...
#include "user/block_bench/executed_transaction_receiver.h"
...
BlockBench::DBUserBase::DBUserBase() :trCount(0) {
    ...
    if (false) {
        // load average
        const auto& blockServerIps = yamlConfig->getBlockServerIPs();
        // bring up receiver from client
        if (!executedTransactionReceiver) {
            executedTransactionReceiver = new ExecutedTransactionReceiver(blockServerIps, yamlConfig->getEpochServerIPs());
        }
        for(const auto& ip: blockServerIps) {
            invokeClient.emplace_back(ip, std::make_unique<ZMQClient>(ip, "8999", zmq::socket_type::pub));
        }
        // we will receive the requests from ExecutedTransactionReceiver
        ...
        // bring up receiver from client
        if (!executedTransactionReceiver) {
            // send to leader of ordering service
            executedTransactionReceiver = new ExecutedTransactionReceiver({yamlConfig->getLocalBlockServerIP()}, {yamlConfig->getEpochServerIPs()[0]});
        }
...
ExecutedTransactionReceiver* BlockBench::DBUserBase::executedTransactionReceiver= nullptr;
