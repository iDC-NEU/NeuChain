...
public:
    explicit AriaCoordinator(epoch_size_t currentEpoch);
    ~AriaCoordinator() override;

    void run() override;
    size_t addTransaction(std::unique_ptr<std::vector<Transaction*>> trWrapper) override;

    CVWrapper startReservation;
    volatile epoch_size_t reservationEpoch = 0;
...
protected:
    ...
    // return nullptr when currentEpoch=i has no pending tx.
