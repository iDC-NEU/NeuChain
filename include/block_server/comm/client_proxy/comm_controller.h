...
public:
    explicit CommController(epoch_size_t startWithEpoch);
...
protected:
    ...
    void blockReceiver(epoch_size_t startWithEpoch);

private:
    std::atomic<bool> finishSignal;
    std::unique_ptr<std::thread> blockReceiverThread;
    BlockingMPMCQueue<Transaction*> receivedTransactionQueue;
