...
public:
    ...
    void initServer() {
        ...
        auto comm = createCommInstanceHandle(blockGenerator->getLatestSavedEpoch() + 1);
        auto* commPtr = comm.get();
        transactionManager->setCommunicateModule(std::move(comm));
        ...
        epochChan = transactionManager->getProcessingEpoch();
        // create ariaCoordinator
        coordinator = yamlConfig->useAggregation() ? std::make_unique<AggregationCoordinator>(epochChan) : std::make_unique<AriaCoordinator>(epochChan);
        commPtr->epochFinishSignal = [&]() {
            coordinator->startReservation.notify_all([&] {
                coordinator->reservationEpoch++;
            });
        };
...
private:
    ...
    std::atomic<epoch_size_t> epochChan{};
};
