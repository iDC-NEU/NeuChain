...
void *ConsensusManager<T>::send_to_raft(void *ptr) {
    ...
    while (!brpc::IsAskedToQuit()) {
        ...
        LOG(INFO) << "batch size: " << self->batch_size << ", dequeue: " << message_count << ", queue size: " << self->request_queue.size_approx();
        // then get content
        auto* requestPtr = new raft::PushRequest;
        for (const auto &requestRaw: requestRawList) {
            requestPtr->add_user_request(requestRaw);
        }
        LOG(INFO) << "send a proposal to raft, flying proposal  count: " << ++cnt;
        clientsPool.execute([self, requestPtr, leaderAddr=leader, &get_leader_by_rpc, &cnt]() {
            ...
            LOG(INFO) << "finished sent a proposal to raft, flying proposal count: " << --cnt;
        });
    }
    return nullptr;
}
